...
import com.tapado.debugmenuDemo.DemoApp
    ...
    Scaffold { paddingValues ->
        Column(
            ...
            Row(
                ...
                val app = remember { context.applicationContext as DemoApp }

                FilledTonalButton(
                    onClick = { app.mockPushService.simulateIncomingPush() },
                ...
                OutlinedButton(
                    onClick = {
                        if (app.mockPushService.isSimulating) {
                            app.mockPushService.stopAutoSimulation()
                        } else {
                            app.mockPushService.startAutoSimulation(5)
                        }
                    },
                    modifier = Modifier.weight(1f)
                ) {
                    Text(
                        if (app.mockPushService.isSimulating) "Stop Auto" else "Auto 5×"
                    )
                }
            }
